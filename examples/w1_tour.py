        ...
        with SqliteStore(path) as store:
            ...
            events = store.append_many(
                [
                    ...
                    tree(
                        TreeEventType.TOOL_RESULT,
                        Source.TOOL,
                        tool_call_id="tc_1",
                        status="executed",
                        result={"p99_ms": 2100, "pool_wait_ms": 1800},
                    ),
                    tree(
                        TreeEventType.AGENT_MESSAGE,
                        Source.AGENT,
                    ...
                    tree(
                        TreeEventType.INTERRUPT,
                        Source.AGENT,
                        interrupt_id="int_1",
                        reason="写操作需人工审批",
                        request={"tool_call_id": "tc_2"},
                    ),
                ]
            )
            store.append(
            ...
            print("== 派生态 ==")
            print(
                f"status={state.status.value} step={state.step} "
            ...
            print("\n== 恢复计划 ==")
            print(
                f"checkpoint={plan.checkpoint.id[:12] if plan.checkpoint else None}… "
            ...
            for event in store.effective_events(branch_id)[:6]:
                print(
                    f"  seq={event.seq:<2} {event.kind.value:<9} {event.type:<14} "

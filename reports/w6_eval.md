...
* **模型出错时，架构决定它会不会变成事故**（weak-guesser 人格，总出诊率 39% 上下）：红线动作执行率 single-shot **20.8%**（其中 2 次是静态拒绝列表里没有的新破坏性动作），harness / langgraph / workflow 均为 0.0%。两条 agent 路线把 54% / 54% 的**出错场景**交给了人工并被拒绝——**gate 的价值不是它自己判断对错，而是它把决定权交到人手里**。

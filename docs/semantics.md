...
| 2026-09-26 | §7-1 / §7-3 的"待 W5"口径已过期：W5–W7 已交付而事项仍未做，改为事实口径并各写出"要做什么才叫做完"（不新增也不删除任何承诺） | 澄清 | §7 |
| 2026-09-26 | 掉电语义口径收口：§3 与 §7-4 写明"要做必须先由所有者立新承诺（`synchronous=FULL` 让每次提交付 fsync 代价）"，本文件维持只承诺 kill -9 语义 | 澄清 | §3、§7 |
...
* 真实掉电语义需要 `synchronous=FULL` + fsync 计时实验，本项目不做，列为
  未来工作——但**它不是一个"顺手做掉"的缺口，而是需要所有者先立一条新承诺**：
  `synchronous=FULL` 会让**每一次提交**都付 fsync 的代价（延迟与吞吐都变），
  也就是说"覆盖掉电语义"会把成本语义一起改掉；这个取舍属于所有者决策，
  不由实现者自行开启。在所有者立下这条新承诺、并给出"实验怎么做才叫做完"
  （fsync 计时口径 + 注入手段：真实断电/虚拟机断电，而不是 mock 掉盘）
  之前，本文件维持"只承诺 kill -9 语义"。
...
1. unknown 集合的收敛条件（何时可判定"不可对账只剩人工"）——W3 给出了产生规则，
   **W5–W7 已交付（场景集、四系统对照、配对统计与门禁都已落地）而本事项仍未做**：
   今天能承诺的只有"unknown 绝不自动重试"，没有任何判据说"到这里就不必再等对账"。
   要做什么才叫做完：给出**可判定的收敛判据与上限**——在哪些条件下
   （探针读回始终为空 / 重试预算耗尽 / 超过某个步数或时限）一条 unknown 不再可能被
   对账消解，以及一个 run 允许残留多少条 unknown（或超时后转人工的规则），
   并配一个能跑出该判据的场景与门禁。
2. ~~租约失效后的接管语义~~：R-B6 已实现最小版（持有/续租/过期失活 + 可读失败），
   **过期接管仍是开放的**（本包只做显式 acquire）。
3. 批量写一半（Saga 补偿）待场景集覆盖——**W5 已交付场景集（64 个合成故障场景）
   而这一类不在里面**，事项仍未做。
   要做什么才叫做完：场景集补一个"批量写一半"的故障类（多步写在中途被杀），
   并写明对账路径——哪一步算已发生、补到哪一步、由谁判（下游读回 / 人工 / 补偿动作），
   且该场景要能被现有四路线跑出逐格结论。
4. `synchronous=FULL` 的掉电语义实验——**要做什么才叫做完**：所有者先立新承诺
   （见 §3：`synchronous=FULL` 让每次提交付 fsync 代价，会一并改掉成本语义），
   再给出 fsync 计时口径与**真实断电**注入手段，并说明它对已有 kill -9 结论的影响面
   （哪些结论可以外推、哪些只能各说各的）。

...
* **缓存纪律值 2 倍成本**：运行时信息进前缀 ⇒ 命中率 57.4% → 0%、成本/调用 +104.6%；
* **卸载是唯一"纯赚"的杠杆**：token/调用 -66%、成本/调用 -62%，且不依赖模型调用；
* **压缩是拿钱买可用性**：能跑完（3 次压缩、0 溢出），但总成本/调用 $0.01431
  （主桶 $0.01073 + 压缩桶 $0.00357）反而最高，命中率只有 35.0%——每次压缩
  都从替换点击穿前缀缓存；
...
（$0.11844；主桶增量 +40%，压缩动作本身占 +39%）；压缩次数与缓存命中率在聚合上反向、

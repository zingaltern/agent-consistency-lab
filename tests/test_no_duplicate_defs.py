...
**修复前会怎样**（`opsenv/systems.py`，W11 之后实测）：`_agent_output_tokens` 与 `_run_state`
各被定义了**两次**，两份函数体逐字相同，第二份静默遮蔽第一份。Python 不报错、
既有用例全绿、门禁全绿——于是"改第一份"不生效（调用拿到的是第二份），
而没有任何机制会发现这件事。这类缺陷的危害不是崩溃，而是**看起来改了、实际没改**：

...
* 6 个命名崩溃窗口 × 16 格 × 5 次 = 80 次真实 SIGKILL + 80 次恢复；
* 8 个长任务变体 × 6 档压缩阈值 = 96 次完整周期，产出三维成本曲线。
...
| 测试 | `203 passed`（约 6 秒）；`tests/test_audit_regressions.py` 是审计回归，`test_stats_and_gates.py`/`test_sweep.py` 是 W6/W7 新增 |
...
| W4 | 缓存纪律值 2 倍成本（+105%）；卸载是唯一纯赚杠杆（-66% token）；压缩与缓存冲突被量化 | `w4-report.md` |
| W5 | 取证充分性决定正确率上限（50% vs 100%）；无 gate 的路线在 20.3% 场景执行破坏性动作，有 gate 的 0%；两条 agent 路线能力等价 | `w5-report.md` |
| W6 | "谁更准"统计不可区分（CI 含 0），"谁更安全"显著（CI [−0.260,−0.146]）；代理指标须先校准（κ）；门禁要能抓"静默丢弃" | `w6-report.md` |
| W7 | 压缩买的是完成率不是省钱（+40%）；压缩次数与缓存命中严格反向；阈值应贴近上限 0.85–0.95 | `w7-report.md` |

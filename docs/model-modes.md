...
1. **live 的能力边界**：本 transport 把视图块拼成一条 user 消息，并以 `tools` 参数下发
   **名字 + 描述 + 参数 schema**（`Tool.parameters`，未声明时回退空参数表），
   并追加一条 live 专用的工具纪律系统消息（`LIVE_TOOL_USE_SYSTEM_PROMPT`）。
   两点实测约束：(a) 空参数表下真实模型倾向于用文字描述工具调用而**不真正调用**
   （DeepSeek 实测如此），因此声明 schema 是 live 可用的前提；
   (b) 即便 schema 齐备，"自主选择工具"仍取决于供应商对工具调用的支持程度。
   接真实模型的用途是**接口回归、录制与语义验证**，不是模型质量评测。
   （2026-09-17 补全：此前只下发名字与描述，见交付报告遗留项。）

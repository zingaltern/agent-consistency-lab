...
> 它不产生结论，也不提升任何结论的强度。接上 MCP 客户端不改变本仓库的性质，
> 更不等于"可用"；本仓库不承诺任何可用性、并发或安全属性。
...
它在 MCP 形态下的**实测**（真 SIGKILL 服务进程 → 重启续跑 → 外部账本判分，含对照组）：

```bash
.venv/bin/python -m integrations.mcp_crash_demo --work-root /tmp/mcp-crash
```

两格只差 `--outbox`（都让下游非幂等、都开探针）：

| 格 | 外部账本 `world.db` | 怎么做到的 |
|---|---|---|
| `--outbox on` | 同一幂等键**恰好 1 行** | 崩溃时留下 `pending` 意图行 → 重启后探针读回"已生效" → 对账收敛，**不重跑** |
| `--outbox off` | 同一幂等键**2 行**（重复） | 没有意图行可对账，恢复只能重跑 |

两格都以退出码 137（128+9，被 SIGKILL 杀死）与 `crash_marker.json` 的
窗口/序号/pid 留下崩溃证据。**只有对照组能让"1 次"有意义**——否则区分不了
"机制在起作用"与"什么都没发生"。
